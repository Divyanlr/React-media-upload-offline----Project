...
export default function UploadArea() {
  ...
  function onComplete(entry) {
    setHistory((h) => {
      const exists = entry.uploadId
        ? h.find((item) => item.uploadId === entry.uploadId)
        : h.find(
            (item) =>
              item.fileName === entry.fileName && item.size === entry.size
          );
      if (exists) return h;
        ...
        <div className="left">
          ...
          >
            ...
            <small className="small">
              Supports image/* and video/* You can select up to 10 files
            </small>
            <input
              ref={inputRef}
              type="file"
          ...
          <div className="file-list">
            ...
            {files.length === 0 && (
              <p className="small">No files selected yet</p>
            )}
          </div>
        </div>
            ...
            <ul>
              {history.map((h, idx) => (
                <li key={idx} className="small">
                  {h.fileName || "unknown"} — {Math.round((h.size || 0) / 1024)}{" "}
                  KB —{" "}
